...
- 支持从设备列表行菜单编辑别名、置顶、打开日志目录和最新日志文件
- 支持从设备列表行菜单将当前屏幕截图直接复制为电脑剪切板图片
- 支持从设备列表行菜单通过 scrcpy 镜像设备画面，或创建独立虚拟屏幕
- 新虚拟屏幕可跟随主屏，或自定义竖屏 `720×1600` / `320 DPI`，并支持填写或筛选启动 App 包名
...
- 支持查看历史日志占用、按设备/时间范围预览清理结果
- 支持独立的应用运行日志，便于排障
- 内置简体中文与英文界面，首次启动按系统语言选择默认值
- 支持应用内检查更新与每日自动检查（每天 08:00 后首次打开窗口时），更新包经签名校验后可一键下载并重启完成升级
- 网络代理配置支持自动读取 `HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY`，或配置不含账号密码的 HTTP / SOCKS5 代理；可检测代理配置，默认请求 GitHub 首页，也可填写 HTTPS 检测地址
...
- `dist/LogcatX.exe`
- `dist/LogcatX_<版本>_windows_x64_portable_<提交号>.zip`

## GitHub Release CI

仓库内置了基于 GitHub Actions 的发布流程：当你推送形如 `v0.7.0` 的 tag 到远端时，会自动：
